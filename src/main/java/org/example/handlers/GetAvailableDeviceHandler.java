...
public class GetAvailableDeviceHandler implements Handler {
    ...
    public void handle(Context context) throws Exception {
        ...
        try{
            ...
            context.getResponse().status(200).send(new ObjectMapper().writeValueAsString(new ResponseDto(true, "Available devices found", devices, null)));
        } catch (SQLException e) {
            log.error(e.toString());
            context.getResponse().status(500).send(new ObjectMapper().writeValueAsString(new ResponseDto(false, "Failed to get available device", null, null)));
        }
    }
}
