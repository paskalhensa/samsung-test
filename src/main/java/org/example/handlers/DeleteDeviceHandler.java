    ...
    public void handle(Context context) throws Exception {
        ...
        context.parse(DeleteDeviceDto.class).then(device -> {
            try {
                ...
                context.getResponse().status(200).send(new ObjectMapper().writeValueAsString(new ResponseDto(true, "Device successfully deleted", null, null)));
            } catch (SQLException e) {
                log.error(e.toString());
                context.getResponse().status(500).send(new ObjectMapper().writeValueAsString(new ResponseDto(false, "Failed to delete device", null, List.of(e.getMessage()))));
            }
        });
    }
}
